...
class MedicationService {
  ...
  Stream<QuerySnapshot> fetchMedicine(DateTime date) {
    ...
    return medicineCollection.doc(firebaseUser!.uid).collection('medicine')
  ...
  createMedicine(Medicine medicine) async {
    DocumentReference docRef = await medicineCollection.doc(firebaseUser!.uid).collection('medicine').add(medicine.toJson());
  ...
  Future<void> updateMedicine(Medicine medicine) async {
    ...
    try {
      await medicineCollection.doc(firebaseUser!.uid).collection('medicine').doc(medicine.id).update({});
      if (kDebugMode) {
  ...
  Future<void> deleteMedicine(String id) async {
    try {
      await medicineCollection.doc(firebaseUser!.uid).collection('medicine').doc(id).delete();
      if (kDebugMode) {
